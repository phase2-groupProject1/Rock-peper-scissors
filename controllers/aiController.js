    ...
    static async playAgainstAI(req, res) {
        try {
            ...
            if (!user_id || !player_move) {
                return res.status(400).json({ error: "User ID and player move are required" });
            }
            ...
            const result = aiResponse.result; // 'win', 'lose', or 'draw'
            const insights = aiResponse.insights; // Hints and insights from AI
            await move.create({
